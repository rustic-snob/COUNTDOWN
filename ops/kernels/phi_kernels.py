...
MODEL = "phi-4"  # or "gemma-2-9b" or "Qwen2.5-14b" or "phi-4"
configs = _configs_map.get(MODEL, _configs_map["phi-4"])
configs_pre_hook = _configs_map_pre_hook.get(MODEL, _configs_map["phi-4"])
...
def phi_dcidx_up(x, w_gate_t, w_up_t, sparse_idx):
...
@torch.compile(fullgraph=True)
def phi_dcidx_down(x, w_down, sparse_idx):
    K, N = w_down.shape
    M = sparse_idx.shape[0]
