...
MODEL = "Qwen2.5-14b"  # or "gemma-2-9b" or "Qwen2.5-14b" or "phi-4"
configs = _configs_map.get(MODEL, _configs_map["Qwen2.5-14b"])
configs_pre_hook = _configs_map_pre_hook.get(MODEL, _configs_map["Qwen2.5-14b"])
...
def qwen_dcidx_up(x, w_gate_t, w_up_t, sparse_idx):
...
@torch.compile(fullgraph=True)
def qwen_dcidx_down(x, w_down, sparse_idx):
    K, N = w_down.shape
    M = sparse_idx.shape[0]
...
def qwen_dcmask_down(
    ...
    kernel = kernel_qwen_dcmask_down
    kernel[grid](
        out,  # data ptrs
        weight,
        x,
