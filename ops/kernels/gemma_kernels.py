...
MODEL = "gemma-2-9b"  # or "gemma-2-9b" or "Qwen2.5-14b" or "phi-4"
configs = _configs_map.get(MODEL, _configs_map["gemma-2-9b"])
configs_pre_hook = _configs_map_pre_hook.get(MODEL, _configs_map["gemma-2-9b"])
...
def gemma_dcidx_up(x, w_gate_t, w_up_t, sparse_idx):
...
@torch.compile(fullgraph=True)
def gemma_dcidx_down(x, w_down, sparse_idx):
    K, N = w_down.shape
    M = sparse_idx.shape[0]
...
def gemma_dcmask_down(
    ...
    kernel = kernel_gemma_dcmask_down
    kernel[grid](
        out,  # data ptrs
        weight,
        x,
